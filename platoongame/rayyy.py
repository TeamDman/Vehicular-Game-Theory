...
import platoonenv
tune.register("Platoon-v0", lambda config: gym.make("Platoon-v0"))
...
parser.add_argument(
    ...
    type=str,
    default="Platoon-v0",
    help="The environment to be trained and evaluated on.",
)
...
if __name__ == "__main__":
    ...
    while num_episodes < args.num_episodes_during_inference:
        ...
        episode_steps += 1
        if episode_steps > 2000 and truncated_already:
            done = True
            print("Forcing end after 2000 steps without done.")
        # Is the episode `done`? -> Reset.
        if truncated and not truncated_already:
            print(f"Episode truncated: Total reward = {episode_reward}, steps = {episode_steps}. Will keep going")
            truncated_already = True
        if done:
