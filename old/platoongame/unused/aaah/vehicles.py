...
class Vehicle:
    ...
    @property
    def risk(self) -> float:
        return sum(vuln.prob * (vuln.severity ** 2) for vuln in self.vulnerabilities)
...
class RubbishVehicleProvider(VehicleProvider):
    ...
    def next(self) -> Vehicle:
        return Vehicle(vulnerabilities=(Vulnerability(0.5,2,CompromiseState.NOT_COMPROMISED),))
...
class RandomVehicleProvider(VehicleProvider):
    ...
    def next(self) -> Vehicle:
        ...
        return Vehicle(vulnerabilities=vulns, in_platoon=False)
...
class JsonVehicleProvider(VehicleProvider):
    ...
    def __init__(self, path: str) -> None:
        ...
        for vehicle in loaded:
            ...
            vehicle = Vehicle(vulnerabilities=tuple(vulns))
